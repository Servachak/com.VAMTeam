...
public class Main {
	...
	public static void main(String[] args) {
		...
		if (month >= 1 && month <= 3) // Q1
			ex.startQuarterReporting(LocalDate.of(2020, 1, 1), LocalDate.now().minusDays(1));
		else if (month >= 4 && month <= 6) // Q2
			ex.startQuarterReporting(LocalDate.of(2020, 4, 1), LocalDate.now().minusDays(1));
		else if (month >= 7 && month <= 9) // Q2
			ex.startQuarterReporting(LocalDate.of(2020, 7, 1), LocalDate.now().minusDays(1));
		else if (month >= 10 && month <= 12) // Q2
			ex.startQuarterReporting(LocalDate.of(2020, 10, 1), LocalDate.now().minusDays(1));
