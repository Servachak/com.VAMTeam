...
import com.reader.AccountsForReport;
import com.reader.Interest;
import com.reader.ReportGenerator;
...
public class ExcelSaveData {
	...
	private void saveAccToSheet(Workbook workbook, ResultSet ACCdata, String ACCname, String currency,
		...
		try {
			...
			while (ex != null) {
				System.out.println("Warning : Cannot write File");
	...
	public void weeklyEntries(String date) {
		...
		try {
			while (!outOfACC) {
				...
				if (connection != null) {
					...
					while (accountsForWeeklyReport.next()) { // If data exist
						outOfACC = false;
						currACCcounter++;
						ResultSet accountNumberData = dataProvider
								.getAccountNumberForReport(accountsForWeeklyReport.getString(2));

						accountNumberData.next();
						String accNumber = accountNumberData.getString(2);
					...
					try {
						...
						System.out.println("Warning : Tread Can't wake up ");
						e.printStackTrace();
					}
					continue;
				}
			...
			System.out.println("Warning : Weekly ");
			System.err.println("SQLException information");
			while (ex != null) {
				System.err.println("Error msg: " + ex.getMessage());
				System.err.println("SQLSTATE: " + ex.getSQLState());
			...
			System.out.println("Warning : Weekly IO ");
	...
	public void saveAccount(Workbook workbook, ArrayList<Interest> ACCdata, String ACCname, String reportName,
			String reportPeriod) {
	...
	public void monthlyReport(LocalDate fromDate, LocalDate toDate) {
		...
		try {
				...
				if (connection != null) {
					...
					while (accountsForMonthlyReport.next()) {
						...
						if (prevACChier == accountsForMonthlyReport.getInt(1)) {// write data to the same hierarchy
							...
							if (accData.size() == 0)
								continue;
							saveAccount(workbook, accData, accNumber, reportName,
			...
			System.out.println("Warning : Monthly SQL");
			System.err.println("SQLException information");
			while (ex != null) {
				System.err.println("Error msg: " + ex.getMessage());
				System.err.println("SQLSTATE: " + ex.getSQLState());
			...
			System.out.println("Warning : Monthly IO)");
			System.err.println("IOException information");
			while (ex != null) {
				System.err.println("Error msg: " + ex.getMessage());
	...
	public void getReport(int reportType, LocalDate fromDate, LocalDate toDate) {
		...
		try {
			while (!outOfACC) {
				...
				if (connection != null) {
					// System.out.println("Still open1");
					ResultSet accountsForReport = dataProvider.getEntriesData(reportType, toDate.toString(),
							currACCcounter);
					...
					while (accountsForReport.next()) {
						...
						if (workbook.getSheet(accNumber) != null) // if we already have sheet with current account name
							continue; // then skip
						if (prevACChier == accountsForReport.getInt(1)) {// write data to the same hierarchy
							// System.out.println("Still open3");
							ArrayList<Interest> accData = report.getReport(accountsForReport.getString(2), fromDate,
									toDate);
							if (accData.size() == 0)
								continue;
							saveAccount(workbook, accData, accNumber, reportName,
			...
			System.out.println("Warning : SQL getReport EXCELSAVE DATA ");
			System.err.println("SQLException information");
			while (ex != null) {
				System.err.println("Error msg: " + ex.getMessage());
				System.err.println("SQLSTATE: " + ex.getSQLState());
			...
			System.out.println("Warning : IO getReport EXCELSAVE DATA ");
			System.err.println("IOException information");
			while (ex != null) {
				System.err.println("Error msg: " + ex.getMessage());
	...
	public void monthlyEntries(String date) {
		...
		try {
				...
				if (connection != null) {
					...
					while (accountsForMonthlyReport.next()) {
						...
						if (prevACChier == accountsForMonthlyReport.getInt(1)) { // write data to the same hierarchy
							...
							prevACChier = accountsForMonthlyReport.getInt(1);
						} else { // create new file for other hierarchy and Save previous file
							System.out.println("**** " + accNumber);
							if (workbook.getNumberOfSheets() != 0) {
					...
					try {
						...
						System.out.println("Warning : Tread monthlyEntries EXCELSAVE DATA ");
						e.printStackTrace();
					}
					continue;
				}
			...
			System.out.println("Warning : SQL monthlyEntries EXCELSAVE DATA ");
			System.err.println("SQLException information");
			while (ex != null) {
				System.err.println("Error msg: " + ex.getMessage());
				System.err.println("SQLSTATE: " + ex.getSQLState());
			...
			System.out.println("Warning : IO monthlyEntries EXCELSAVE DATA ");
			System.err.println("IOException information");
			while (ex != null) {
				System.err.println("Error msg: " + ex.getMessage());
	...
	private ResultSet getSettlement(String account) {
		...
		while (account != null) {
			...
			try {
				...
				System.out.println("Warning : getSettlement EXCELSAVE DATA ");
				e.printStackTrace();
			}
		}
		return settlement;
